...
*/
	...
	@Test
	public void 未ログインの場合は予約フォームの送信後に予約内容の確認ページにリダイレクトする() throws Exception {
	...
	public void ログイン済みの場合は予約フォームの送信後に予約内容の確認ページにリダイレクトする() throws Exception {
		mockMvc.perform(post("/houses/2/reservations/input").with(csrf())
				.param("checkinDate", "2024-04-01")
				.param("checkoutDate", "2024-04-02")
				.param("numberOfPeople", "1"))
				.andExpect(status().is3xxRedirection())
	...
	public void ログイン済みの場合は予約後に会員用の予約一覧ページにリダイレクトする() throws Exception {
		...
		//セッションを作成し、ReservationDTOオブジェクトを保存する
		MockHttpSession mockHttpSession = new MockHttpSession();
		ReservationDTO reservationDTO = new ReservationDTO(2, LocalDate.parse("2024-04-01"),
				LocalDate.parse("2024-04-02"), 2, 6000);
		mockHttpSession.setAttribute("reservationDTO", reservationDTO);
		...
		assertThat(reservation.getHouse().getId()).isEqualTo(2);
		...
		assertThat(reservation.getNumberOfPeople()).isEqualTo(2);
