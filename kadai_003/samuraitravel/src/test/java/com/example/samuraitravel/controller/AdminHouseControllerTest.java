...
public class AdminHouseControllerTest {
	...
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿一覧ページが表示されず403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses"))
	...
	@Test
	@WithUserDetails("taro.samurai@example.com")
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿ページが表示されずに403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses/1"))
				.andExpect(status().isForbidden());
	}
	...
	@Test
	@WithUserDetails("taro.samurai@example.com")
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿登録ページが表示されず403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses/register"))
				.andExpect(status().isForbidden());
	}
    ...
    @WithUserDetails("taro.samurai@example.com")
    @Transactional
    public void 一般ユーザーとしてログイン済みの場合は民宿を削除せず403エラーが発生する() throws Exception {
        mockMvc.perform(post("/admin/houses/1/delete").with(csrf()))
               .andExpect(status().isForbidden());
